<script lang="ts">
	...
	function setFavicon(isLight: boolean) {
		...
		link.rel = "icon";
		link.href = isLight ? "/favicon/favicon-black.ico" : "/favicon/favicon-white.ico";
	...
	{#if $sessionLoaded}
	  {#if $isAuth}
		<div class="idinfo">
			...
			>
			{#if $userData?.uuid && !copied}
				{($userData.uuid).slice(0,8)}...
			{/if}
			</button>
			{#if copied}<span class:copied={copied}>{t('copied', currentLang)}</span>{/if}
		</div>
		<span> | </span>
...
<section class="legal-buttons">
	<a class:active={$page.url.pathname === '/privacy'} href="/privacy">Privacy</a>
	<span> | </span>
	<a class:active={$page.url.pathname === '/terms'} href="/terms">Terms</a>
</section>

<section class="support">
	<a href="{"mailto:" + t('common.contact.email_value', currentLang)}">{t('support', currentLang)}</a>
	<a href="https://github.com/fogvpn" target="_blank" rel="noopener noreferrer">
	...
	<a href={t('common.contact.telegram_url', currentLang)} target="_blank" rel="noopener noreferrer">
